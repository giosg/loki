...
const insertAfter = (content, pattern, stringToInsert, fallback = "prepend") => {
  const match = content.match(pattern);
  if (match) {
    const insertionIndex = match.index + match[0].length;
    return `${content.substr(0, insertionIndex)}\n${stringToInsert}${content.substr(
      insertionIndex,
    )}`;
  }
  if (fallback === "append") {
...
function init(args) {
  ...
  if (pkg.loki && !force) {
    die("Loki already configured, re-run with --force to force reconfiguration");
  }
  ...
  if (isReactNativeProject) {
    ...
    if (storybookjs.indexOf("loki/configure-react-native") !== -1) {
      warn(`${relative(storybookjsPath)} already has loki configuration, skipping...`);
    } else {
    ...
    const configjsPath = `${storybookPath}/config.ts`;
    ...
    if (configjs.indexOf(configPackage) !== -1) {
      warn(`${relative(configjsPath)} already has loki configuration, skipping...`);
    } else {
